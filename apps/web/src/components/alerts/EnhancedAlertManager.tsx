...
export const EnhancedAlertManager: React.FC<EnhancedAlertManagerProps> = ({ 
  ...
  const loadAlerts = async () => {
    ...
    try {
      ...
      const typedAlerts = (data || []).map(alert => ({
        ...alert,
        alert_type: alert.alert_type as 'price_cross' | 'percentage_change' | 'volume_spike' | 'trailing_stop'
      }));
  ...
  const triggerAlert = async (alert: Alert) => {
    try {
      ...
      const alertMessage = alert.alert_type === 'price_cross'
    ...
    try {
      ...
      if (data?.enabled) {
        ...
        setTimeout(() => {
          ...
          oscillator2.frequency.setValueAtTime(frequencies[data.sound_type] || 880, audioContext.currentTime);
          oscillator2.type = 'sine';
          
          gainNode2.gain.setValueAtTime(0, audioContext.currentTime);
          gainNode2.gain.linearRampToValueAtTime(data.volume * 0.3, audioContext.currentTime + 0.01);
  ...
  const addAlert = async () => {
    ...
    if (alertType === 'percentage_change' && (!percentageThreshold || isNaN(parseFloat(percentageThreshold)))) {
      toast({
    ...
    try {
      ...
      const typedAlert = {
        ...data,
        alert_type: data.alert_type as 'price_cross' | 'percentage_change' | 'volume_spike' | 'trailing_stop'
      };
    ...
    return (
      <Card className="bg-gray-900 border-gray-700 p-4">
        <div className="text-center py-4">
          ...
          <p className="text-gray-400">Sign in to create price alerts</p>
        </div>
      </Card>
    );
  }
                  ...
                  <div className="flex items-center gap-2">
                    {alert.alert_type === 'price_cross' ? (
                      ...
                      <>
                        ...
                        <span className="text-white text-sm">
                          {alert.is_above ? '+' : '-'}{alert.percentage_threshold}%
                        </span>
                      </>
                    )}
